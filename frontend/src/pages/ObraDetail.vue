...
const handleVincularChecklist = (formData) => {
  ...
  // Adicionar checklist à lista
    ...
    <div class="bg-card rounded-lg border border-border p-6 shadow-sm mb-8">
      <div class="flex justify-between items-center mb-4">
        <h2 class="text-lg font-semibold text-foreground">Checklists Vinculados</h2>
        <Button @click="showVincularChecklist" variant="secondary" size="sm">
          + Vincular Checklist
        </Button>
      </div>
      ...
      <div v-else class="text-center py-8">
        <p class="text-muted-foreground mb-4">Nenhum checklist vinculado</p>
        <Button @click="showVincularChecklist" variant="secondary">
          Vincular Primeiro Checklist
        </Button>
      </div>
    </div>
...
const showVincularChecklist = () => {
  alert('Implementar modal de vincular checklist')
}

const startInspection = (checklistId) => {
  alert('Implementar iniciar inspeção para checklist ' + checklistId)
}
</script>
